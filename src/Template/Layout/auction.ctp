...
<body>
    <nav class="top-bar titlebar" data-topbar role="navigation">
        ...
        <div class="top-bar-section">
            <ul class="right">
                <li><a target="_blank" href="https://plus.goolge.com/+TuyanoSYODA">about</a></li>
            </ul>
        </div>
    </nav>
    <?=$this->Flash->render() ?>
    <div class="container clearfix">
        ...
        <nav class="large-2 medium-3 columns sidebar" id="actions-sidebar">
            <ul class="side-nav">
                ...
                <li><?php if ($authuser['role'] === 'admin') {
                        echo $this->Html->link(__('管理画面'), ['controller'=>'Users',
                                'action'=>'index']);
                }
                ?></li>
                <li><?=$this->Html->link(__('ログアウト'), 
                ['controller'=>'AuctionBase','action' => 'logout']) ?></li>
            </ul>
        </nav>
    </div>
    <footer>
