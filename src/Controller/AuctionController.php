...
class AuctionController extends AuctionBaseController
{
    ...
    public function bid($biditem_id = null)
    {
         ...
         $bidrequest->biditem_id = $biditem_id; // 入札する商品のID・・・$biditem_idはどこから来ている？
         $bidrequest->user_id = $this->Auth->user('id'); // ログインユーザーのID
         // POST送信時の処理
         if ($this->request->is('post')) {
    ...
    {
        // フォームに設定するためのBidmessagesのインスタンスを新たに用意
        ...
        try { // $bidinfo_id から Bidinfoを取得する
            $bidinfo = $this->Bidinfo->get($bidinfo_id, ['contain'=>['Biditems']]);
        } catch(Exception $e) { // 誰も落札していなければBidinfoは無い。Bidinfoが存在しないケースを考える。
            $bidinfo = null;
        }
        // Bidmessageをbidinfo_idとuser_idで検索  やりとりしたメッセージをまとめて取り出す。
